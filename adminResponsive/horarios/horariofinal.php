...
<body>
			...
			<ul class="list-unstyled full-box dashboard-sideBar-Menu">
				...
				<li>
					...
					<ul class="list-unstyled full-box">
						<li>
							<a href="horariofinal.pfp"><i class="zmdi zmdi-book zmdi-hc-fw"></i> Horarios de Examenes Finales</a>
						</li>
						<li>
	...
	<section class="full-box dashboard-contentPage">
		...
		<div class="container-fluid">
			<div class="page-header">
			  <h1 class="text-titles" id="forms"><i class="zmdi zmdi-file-text"></i> HORARIO EXAMEN FINAL
					...
					<div class="table-responsive">
						...
						<table class="table table-hover text-center">
							<?php
								...
								left join e_final hor on mat.idmaterias=hor.idmaterias where mat.idcarreras=1 order by mat.idmaterias;";
								...
								if(mysqli_num_rows($res)>0){
									...
									echo "<th class='text-center'>OPORTUNIDAD</th>";
									echo "<th class='text-center'>FECHA</th>";
									echo "<th class='text-center'>HORARIO</th>";
									...
									while($reg=mysqli_fetch_array($res)){
										...
										<td><select  name='oportunidad".$reg["idmaterias"]."' id='oportunidad".$reg["idmaterias"]."'>
											...
											if($reg['oportunidad'] == 'Primera'){ echo 'selected'; }
											echo ">Primera</option>
											<option value='Segunda' ";
											if($reg['oportunidad'] == 'Segunda'){ echo 'selected'; }
											echo ">Segunda</option>
											<option value='Tercera' ";
											if($reg['oportunidad'] == 'Tercera'){ echo 'selected'; }
											echo ">Tercera</option>
										</select></td>
										...
										if (isset($reg["ide_final"])) {
											echo "<section id='agregarmodificar".$reg["idmaterias"]."'><button class='btn btn-outline-success btn-sm' onclick='modificarhorariofinal(".$reg["idmaterias"].")'><i class='zmdi zmdi-refresh zmdi-hc-lg'></i></button></section>";
											
										}else{
											echo "<section id='agregarmodificar".$reg["idmaterias"]."'><button class='btn btn-outline-success btn-sm' onclick='registrarhorariofinal(".$reg["idmaterias"].")'><i class='zmdi zmdi-check zmdi-hc-lg'></i></button></section>";
										}
							...
							?> 
							<script>
								function registrarhorariofinal(id) {
									var id;
									var fecha= $('#fecha'+id).val();
									var horario=$('#horario'+id).val();
									var oportunidad=$('#oportunidad'+id).val();
									...
									$.ajax({
										url : 'registrarhorariofinal.php',
										type : 'POST',
										dataType : 'html',
										data : {materias: id, fecha: fecha,horario: horario,oportunidad: oportunidad,aula: aula,carrera: carrera,semestre: semestre,profesor: profesor},
									})
									.done(function(resultado) {
										var a ="#agregarmodificar"+resultado;
										//alert(a);
										$(a).html("<button class='btn btn-outline-success btn-sm' onclick='modificarhorariofinal("+resultado+")'><i class='zmdi zmdi-refresh zmdi-hc-lg'></i></button>");
										if (resultado==id) {
								...
								function modificarhorariofinal(id) {
									var id;
									var fecha= $('#fecha'+id).val();
									var horario=$('#horario'+id).val();
									var oportunidad=$('#oportunidad'+id).val();
						...
						<section id="matcivil">
							...
							<?php
								...
								hor.fecha,hor.oportunidad,hor.aula,hor.horario,sem.semestre
								 from materias mat inner join carreras carr on carr.idcarreras = mat.idcarreras 
								inner join semestre sem on sem.idSemestre=mat.idSemestre
								left join e_final hor on mat.idmaterias=hor.idmaterias where mat.idcarreras=2 order by mat.idmaterias;";
						...
						<section id="matelectricidad">
							...
							<?php
								...
								hor.fecha,hor.oportunidad,hor.aula,hor.horario,sem.semestre
								 from materias mat inner join carreras carr on carr.idcarreras = mat.idcarreras 
								inner join semestre sem on sem.idSemestre=mat.idSemestre
								left join e_final hor on mat.idmaterias=hor.idmaterias where mat.idcarreras=3 order by mat.idmaterias;";
						...
						<section id="matelectronica">
							...
							<?php
								...
								hor.fecha,hor.oportunidad,hor.aula,hor.horario,sem.semestre
								 from materias mat inner join carreras carr on carr.idcarreras = mat.idcarreras 
								inner join semestre sem on sem.idSemestre=mat.idSemestre
								left join e_final hor on mat.idmaterias=hor.idmaterias where mat.idcarreras=4 order by mat.idmaterias;";
	...
	<script src="consultahorariofinal.js"></script>
